...
// Export physical page
export const ProductListPage: FC = () => {
  // States
  const [physicalProducts, setPhysicalProducts] = useState<IProduct[]>([]);
  ...
  const fetchProducts = () => {
    ...
    ProductsApi.readAll({
      ...
      .then((res) => {
        setPhysicalProducts(res.products.map((product) => new ProductModel(product)));
  ...
  // New handler
  const handleNew = () => {
    navigate(ROUTES.PHYSICAL_PRODUCT.NEW);
  };

  // Routing handler
  const orderDetailHandler = () => {
    if (orderId) {
      navigate(ROUTES.PHYSICAL_PRODUCT.DETAIL.replace(':id', orderId));
    }
  };
  ...
  const handleEdit = (id: string) => {
    navigate(ROUTES.PHYSICAL_PRODUCT.EDIT.replace(':id', id));
  };
  ...
  const handleView = (id: string) => {
    navigate(ROUTES.PHYSICAL_PRODUCT.DETAIL.replace(':id', id));
  };
  ...
  const columns: IColumn[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <Stack direction="row" spacing={4}>
          {console.log('reo', row)}
            <Chip label={row.artistDetails?.name} />
        </Stack>
      )
    },
    {
  ...
  return (
    <>
      ...
      <ConfirmDialog
        title={'Alert'}
        description="You can not remove the product. Because there is an order which is bind with the current song."
