...
export const VideoListPage: FC = () => {
  ...
  const fetchProducts = () => {
    ...
    ProductsApi.readAll({
      ...
      .then((res) => {
        setVideos(res.products.map((product) => new ProductModel(product)));
  ...
  // New handler
  const handleNew = () => {
    navigate(ROUTES.VIDEO.NEW);
  };

  // Routing handler
  const orderDetailHandler = () => {
    if (orderId) {
      navigate(ROUTES.VIDEO.DETAIL.replace(':id', orderId));
    }
  };
  ...
  const handleEdit = (id: string) => {
    navigate(ROUTES.VIDEO.EDIT.replace(':id', id));
  };
  ...
  const handleView = (id: string) => {
    navigate(ROUTES.VIDEO.DETAIL.replace(':id', id));
  };
      ...
      render: (row) => (
        <Stack direction="row" spacing={4}>
          {row.categoryNames.map((category, index) => (
            <Chip key={index} label={category} sx={{ backgroundColor: COLORS.BLUE, color: 'white' }} />
          ))}
        </Stack>
      )
    },
  ...
  return (
    <>
      ...
      <ConfirmDialog
        title={'Alert'}
        description="You can not remove the video. Because there is an order which is bind with the current song."
