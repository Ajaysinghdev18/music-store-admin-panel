...
export const NewSongPage: FC = () => {
  ...
  if (location.pathname.includes('/songs') && location.pathname.includes('/edit')) {
  ...
  const fetchProduct = () => {
    if (!id) {
      CategoriesApi.readAll()
        .then((res) => {
          setProduct({ ...res.product, category: res.categories.map((cat) => cat.id) });
        });
    } else {
      setLoading(true);
      ProductsApi.read(id)
  ...
  return (
    <Fragment>
      <Box sx={{ overflow: 'visible', paddingX: '1rem' }}>
        {id && !product ? (
